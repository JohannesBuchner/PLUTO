...
int main (int argc, char** argv)
{
  ...
  {
    const char* in_file = "ribbonTorus.inputs";
    ...
    const char* basename = "ribbonTorus";
                     ...
                     Real&     a_dx)
{
 ...
 CH_assert(n_cell.size() == SpaceDim);
  ...
  bool inside = true;
  EllipsoidIF ellipsoid(radii,zero,inside);

  TransformIF rotate(ellipsoid);

  RealVect rotAxis(D_DECL(0.0,0.0,1.0));
  rotate.rotate(M_PI/6.0,zero,rotAxis);
  rotate.translate(center);

  LatheIF implicit(rotate,insideRegular);
