...
int main (int argc, char** argv)
{
  ...
  {
    const char* in_file = "cylinder.inputs";
                     ...
                     Real&     a_dx)
{
 ...
 CH_assert(n_cell.size() == SpaceDim);
  ...
  pp.getarr("point",vectorPoint,0,SpaceDim);

  for (int idir = 0; idir < SpaceDim; idir++)
  {
    point[idir] = vectorPoint[idir];
  }
  ...
  TiltedCylinderIF implicit(radius,direction,point,insideRegular);
