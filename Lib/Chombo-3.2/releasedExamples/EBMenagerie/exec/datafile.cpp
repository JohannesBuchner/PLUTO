...
int main (int argc, char** argv)
{
  ...
  {
    const char* in_file = "datafile.inputs";
                     ...
                     Real&     a_dx)
{
 ...
 CH_assert(n_cell.size() == SpaceDim);
  ...
  char name[1000];
  sprintf(name,"%s%dd.ASCII",basename,SpaceDim);
  DataFileIF implicit(name,DataFileIF::ASCII,level,insideRegular);
