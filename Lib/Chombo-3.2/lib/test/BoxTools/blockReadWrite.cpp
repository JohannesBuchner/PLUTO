...
static const char *pgmname = "blockReadWrite" ;
...
blockTest()
{
  ...
  //output data to file
  string filename("blockfile.hdf5");
  HDF5Handle handleOut(filename.c_str(),  HDF5Handle::CREATE);

  blockWriteLevel(handleOut, lev, dataOut, dum, dum, dum, domain, ref, nghost, interv);
  handleOut.close();
  ...
  blockReadLevel( handleIn,  lev, dataIn,  dum, dum, dum, domain, ref,  interv, true);
  handleIn.close();
