...
; --------------------------------------------------------
; 3. Allocate memory for an array of particles structures
; --------------------------------------------------------
  particles = 0.0;  Free memory (?)
  particles = REPLICATE({particle_struct},nparticles)

; ---------------------------------------------------------------
; 4. Read data (using assoc):
; ---------------------------------------------------------------
  POINT_LUN,-unit,pos
  IF (KEYWORD_SET(FLOAT)) THEN BEGIN
...
; ---------------------------------------------------------------
; 5. Assign array value to structure members.
; ---------------------------------------------------------------
  FOR n = 0,nfields-1 DO BEGIN
    IF (field_names[n] EQ "id") THEN particles.id  = REFORM(Q[n,*]); 
    IF (field_names[n] EQ "x1") THEN particles.x1  = REFORM(Q[n,*]); 
    IF (field_names[n] EQ "x2") THEN particles.x2  = REFORM(Q[n,*]); 
    IF (field_names[n] EQ "x3") THEN particles.x3  = REFORM(Q[n,*]); 
    IF (field_names[n] EQ "vx1") THEN particles.vx1 = REFORM(Q[n,*]); 
    IF (field_names[n] EQ "vx2") THEN particles.vx2 = REFORM(Q[n,*]); 
    IF (field_names[n] EQ "vx3") THEN particles.vx3 = REFORM(Q[n,*]); 
    IF (field_names[n] EQ "color")  THEN particles.color = REFORM(Q[n,*]); 
    IF (field_names[n] EQ "tinj")   THEN particles.tinj = REFORM(Q[n,*]); 
    IF (field_names[n] EQ "energy") THEN particles.energy = REFORM(Q[n,*]); 
  ENDFOR
...
; ---------------------------------------------------------
  ...
  struct1 = {particle_struct, id: 0.d0, x1:  0.d0,  x2: 0.d0, x3: 0.d0,    $
